...
teste <- hpfilter(Yrwindow, type="lambda", freq = 14400)
datas <- seq(from=as_date(paste0(Y2start[1],"-",Y2start[2],"-01")), to=as_date(paste0(Y2final[1],"-",Y2final[2],"-01")), by='month')
#df <- data.frame(abs=Y2rwindow, z=as.data.frame(teste$cycle),Data=datas)
#filtro <- ggplot(data=df, aes(x=Data,y=100*abs))+
# geom_line()+
# geom_line(linetype=2,aes(y=100*Series.1),color="red")
...
save.image(file=paste0('HSrfcomplete', nforecast,'hpstatcycleforecasts.RData'))

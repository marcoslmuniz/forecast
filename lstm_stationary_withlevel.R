...
for(fh in 1:dim(as.matrix(horizonvector))[1]){
  ...
  for(i in 1:((nforecast-forecasthorizon))){
    ...
    X <- cbind(XeZ[1:(nrow(XeZ)-forecasthorizon),],Y[1:(nrow(Y)-forecasthorizon)])
    ...
    Z <- cbind(XeZ[((dim(XeZ)[1]+1-batch_size):dim(XeZ)[1]),],Y[((length(Y)+1-batch_size):length(XeZ))])    # Z tem comprimento do batchsize para poder servir como input na função 'predict'
    ...
    for(j in 1:(dim(XeZ)[1])){
      input <- sigmoid(t(Whi)%*%hmenos1 + t(Wxi)%*%Xht[j,] + Bi)
      forget <- sigmoid(t(Whf)%*%hmenos1 + t(Wxf)%*%Xht[j,] + Bf)
      ctio <- tanh(t(Whc)%*%hmenos1 + t(Wxc)%*%Xht[j,] + Bc)
      output <- sigmoid(t(Who)%*%hmenos1 + t(Wxo)%*%Xht[j,] + Bo)
  ...
  save.image(file=paste0('loop1round', fh,'.RData'))  
}
...
for(fh in 1:length(horizonvector)){
  ...
  for(j in 1:(dim(lags)[1])){
    ...
    for(i in 1:((nforecast-forecasthorizon))){
      ...
      Xrwindow <- cbind(Xrwindow,X2window,Yrwindow)
      ...
      XAR <- cbind(XAR,Yrwindow[-(c((1:(forecasthorizon+lags[j]-1)),(length(Yrwindow)+1-forecasthorizon):length(Yrwindow)))])
      Y2AR<- Y2window[-(1:(lags[j]-1+forecasthorizon))]
      ZAR <- embed(Y2window,lags[j])[(dim(as.matrix(Y2window))[1]+1-lags[j]),]
      ZAR <- cbind(ZAR,Yrwindow[length(Yrwindow)])
      ARmodel <- lm(Y2AR ~ XAR)
  ...
  save.image(file=paste0('loop2round', fh,'.RData'))
}

end_time <- Sys.time()
end_time - start_time

save.image(file=paste0('HSrfcomplete', nforecast,'statwithlevelforecasts.RData'))

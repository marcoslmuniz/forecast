...
for(i in 1:((nforecast-forecasthorizon))){
  ...
  datas <- seq(from=as_date(rollingwindow[i,1]), to=as_date(rollingwindow[i+forecasthorizon-1,2]), by='month')
  ...
  Y2window <- teste$trend-lag(teste$trend,-forecasthorizon)
...
toc()
save.image(file=paste0('TREND',forecasthorizon, 'fhorizon.RData'))
...
for(j in 1:(dim(lags)[1])){
  ...
  for(hs in 1:(nforecast-forecasthorizon+1-lags[j])){
    ...
    Yrwindow <- window(unemployment.ts,start=c(year(ymd(rollingwindow[hs+lags[j]+dropout-1,1])),month(rollingwindow[hs+lags[j]+dropout-1,1])),end=c(year(ymd(rollingwindow[hs+forecasthorizon-1,2])),month(rollingwindow[hs+forecasthorizon-1,2])))
    ...
    teste <- hpfilter(Yrwindow, type="lambda", freq = 14400)
    datas <- seq(from=as_date(rollingwindow[hs+lags[j]+dropout-1,1]), to=as_date(rollingwindow[hs+forecasthorizon-1,2]), by='month')
    df <- data.frame(abs=Yrwindow, z=as.data.frame(teste$trend),Data=datas)
    #filtro <- ggplot(data=df, aes(x=Data,y=100*abs))+
    # geom_line()+
    # geom_line(linetype=2,aes(y=100*Series.1),color="red")
...
Xrwindow <- t(na.omit(t(window(mccrack.ts, start=c(year(ymd(rollingwindow[i,1])),month(rollingwindow[i,1])),end=c(year(ymd(rollingwindow[i,2])),month(rollingwindow[i,2]))))))
Yrwindow <- window(unemployment.ts,start=c(year(ymd(rollingwindow[i+lags[j]-1,1])),month(rollingwindow[i+lags[j]-1,1])),end=c(year(ymd(rollingwindow[i+forecasthorizon-1,2])),month(rollingwindow[i+forecasthorizon-1,2])))
...
teste <- hpfilter(Yrwindow, type="lambda", freq = 14400)
datas <- seq(from=as_date(rollingwindow[i+lags[j]-1,1]), to=as_date(rollingwindow[i+forecasthorizon-1,2]), by='month')
df <- data.frame(abs=Yrwindow, z=as.data.frame(teste$trend),Data=datas)
#filtro <- ggplot(data=df, aes(x=Data,y=100*abs))+
# geom_line()+
# geom_line(linetype=2,aes(y=100*Series.1),color="red")
...
save.image(file=paste0('HSrfcomplete', nforecast,'tendforecasts.RData'))
